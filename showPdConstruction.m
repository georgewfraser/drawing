...
for day=1%:length(coeff)
    ...
    nPlots = 20;%sum(ismember(commonFields,controlFields))*2;
    plotRow = 0;
    clf;
    for unit=find(ismember(commonFields,controlFields))'
        ...
        subplot(nPlots/4,4,(plotRow-1)*2+1);
        plotGroup(R_c, L_c, C, meanR_c, unit, shift, ymax)
        subplot(nPlots/4,4,(plotRow-1)*2+2);
        plotGroup(R_p, L_p, C, meanR_p, unit, shift, ymax)
    end
    set(gcf,'PaperPosition',[0 0 6.9 nPlots/2*6.9/8]);
