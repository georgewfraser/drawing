...
for day=1:length(controlPd)
    ...
    fields = intersect(fields, fieldnames(controlEmpiricalPd{day}));
    fields = intersect(fields, fieldnames(perturbEmpiricalPd{day}));
    controlTh{day} = struct();
    perturbTh{day} = struct();
    decoderChange{day} = struct();
    for iif=1:length(fields)
        f = fields{iif};
        controlTh{day}.(f) = cart2pol(controlEmpiricalPd{day}.(f)(1), controlEmpiricalPd{day}.(f)(2));
        perturbTh{day}.(f) = cart2pol(perturbEmpiricalPd{day}.(f)(1), perturbEmpiricalPd{day}.(f)(2));
    ...
    mc = nanmean(wrapToPi(cell2mat(struct2cell(perturbTh{day}))-cell2mat(struct2cell(controlTh{day}))));
    for iif=1:length(fields)
